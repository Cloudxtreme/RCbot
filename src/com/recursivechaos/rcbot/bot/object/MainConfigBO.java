	...
	public MainConfigBO(String fileURL) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {	 
				...
				botSettings.setRedditPreview(Boolean.valueOf(node.getChildText("redditpreview")));
				botSettings.setNewUserGreeting(Boolean.valueOf(node.getChildText("newusergreeting")));
				botSettings.setCalendar(Boolean.valueOf(node.getChildText("calendar")));
				botSettings.setCalendarUrl((node.getChildText("calendarurl")));
				botSettings.setDice(Boolean.valueOf((node.getChildText("dice"))));
				botSettings.setCatfacts(Boolean.valueOf((node.getChildText("catfacts"))));
				botSettings.setDadjokes(Boolean.valueOf((node.getChildText("dadjokes"))));
				botSettings.setDadjokeChance(Integer.parseInt((node.getChildText("dadjokechance"))));
